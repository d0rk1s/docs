...
description: "Sistema de advertencias tempranas para cierres que empiezan en los próximos 14 días"
---
...
<Note>
**Configuración**: El período de 14 días puede ser ajustado por administradores del sistema si su clínica requiere un período diferente.
</Note>
...
<Frame>
  <img src="/images/screenshots/closures_upcoming_widget.png" alt="Widget de próximos cierres en el dashboard con countdown y alertas" />
</Frame>
...
Endpoint API que retorna estado completo de la clínica:
...
La página de gestión de cierres incluye una sección "Upcoming":
...
<Steps>
  ...
  <Step title="Revisar lista de cierres próximos">
    El sistema mostrará todos los cierres que empiezan en los próximos 14 días:

    **Si hay cierres próximos**:
...
<AccordionGroup>
  <Accordion title="Ejemplo 1: Vacaciones próximas (7 días)">
  ...
  <Accordion title="Ejemplo 2: Permiso médico urgente (2 días)">
    **Escenario**: Hoy es 01/02/2026, permiso médico empieza el 03/02/2026.
  ...
  <Accordion title="Ejemplo 3: Capacitación futura (12 días)">
    **Escenario**: Hoy es 01/03/2026, capacitación empieza el 13/03/2026.
  ...
  <Accordion title="Ejemplo 4: Múltiples cierres solapados">
    **Escenario**: Dos cierres próximos en fechas cercanas.
...
El período de 14 días puede ser ajustado según las necesidades de su clínica. Contacte a soporte si necesita modificar este período.

**Valores recomendados**:
- **7 días**: Para clínicas pequeñas con planificación semanal
- **14 días**: Balance entre relevancia y anticipación (predeterminado)
- **30 días**: Para clínicas grandes con planificación mensual

<Warning>
Aumentar a más de 30 días puede llenar la lista con cierres muy lejanos que no son relevantes.
</Warning>
...
<AccordionGroup>
  <Accordion title="No veo upcoming closures pero tengo cierres creados">
    **Problema**: Creé cierres, pero no aparecen en "Upcoming Closures".

    **Causas posibles**:

    **1. Cierres empiezan en &gt;14 días**:
  ...
  <Accordion title="Veo cierres que ya pasaron en upcoming closures">
    ...
    **Causa**: Problema en el sistema.

    **Solución**:
    1. Refrescar la página (Ctrl+F5)
    2. Si persiste, contacte a soporte con los detalles de los cierres afectados
  </Accordion>

  <Accordion title="Countdown no se actualiza en tiempo real">
    **Problema**: "Days until start" muestra "7 días" pero ya pasó un día.

    **Causa**: El countdown se calcula al cargar la página, no se actualiza automáticamente.

    **Solución**:
    - Refrescar la página para ver el countdown actualizado
    - Recomendación: Revisar upcoming closures diariamente para tener la información más actualizada
  </Accordion>

  <Accordion title="¿Cómo ver cierres de &gt;14 días?">
    **Problema**: Quiero ver cierres que empiezan en 30 días.

    **Solución**:
    1. Navega a lista completa de cierres: Gestión → Cierres → "Todos"
    2. Filtra por rango de fechas para ver cierres más lejanos
    3. Ordena por fecha de inicio

    **Alternativa**:
    - Contacta a soporte para solicitar un ajuste en el período de visualización
  </Accordion>
</AccordionGroup>
...
Crea cierres con al menos 14 días de anticipación:
...
<Warning>
**Atención a cierres muy cercanos (&lt;3 días)**:

Si ves cierres con countdown de 1-2 días:
...
Los upcoming closures solo bloquean NUEVAS citas. Si ya hay citas agendadas en ese período, NO se cancelan automáticamente. Debes manejarlas manualmente.
</Note>
...
<CardGroup cols={2}>
  <Card title="Crear Cierre" icon="calendar-plus" href="/closures/create-closure">
    Crear nuevo cierre con anticipación
  </Card>
